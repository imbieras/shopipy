...
export default function ScheduleService({ selectedService, onBack }) {
  ...
  const { data: timeSlots = [], isLoading: isSlotsLoading } = useQuery({
    queryKey: ['timeSlots', businessId, selectedStaff, selectedService?.serviceId, selectedDate],
    queryFn: async () => {
    ...
    enabled: !!(businessId && selectedStaff && selectedService?.serviceId && selectedDate)
  });
